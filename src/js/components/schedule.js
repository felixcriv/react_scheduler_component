...
var Schedule = React.createClass({
  ...
  handleClick: function(e) {
    ...
    if (this.refs[slot].getDOMNode().children[1]) {
      editingMode = true;
      patient.name = this.refs[slot].getDOMNode().children[1].children[1].innerHTML;
      patient.phone = this.refs[slot].getDOMNode().children[1].children[3].innerHTML;
    }
      ...
      var _slots = function(id, h, handleClick) {
        ...
        //days with appointments
          var spanActive = Object.keys(days).map(function(appointment) {
            ...
            if (_activeClass) {
              _s = <span className = 'active'> {days[appointment]}<hr/><p className = 'hideNumber'>{patients[days[appointment]]}</p></span>
            }
          ...
          return (<td ref= {h.hour + day} className = 'cell' onClick = {
            handleClick.bind(null, {hour: h.hour,day: i})}> {spanActive}</td>)
          }.bind(this));
    ...
    var _schedule = this.props.workingHours.map(function(h, i) {
      return (<tr key = {i}><td>{h.text}</td>{_slots(i, h, this.handleClick)}</tr>)
    }.bind(this));

      return (<span>{_schedule}</span>)
  }
});
